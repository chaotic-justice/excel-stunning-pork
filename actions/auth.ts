...
export async function generateCfToken() {
  ...
  const jwt = await new jose.SignJWT({ sub: process.env.CF_SUB, snack: process.env.CF_SNACK }).setProtectedHeader({ alg }).setIssuedAt().setIssuer("excel-peachy").setExpirationTime("2m").sign(secret)
